  ...
  private suspend fun setItemImpl(key: String, value: String?, options: SecureStoreOptions, keyIsInvalidated: Boolean) {
    ...
    try {
      if (keyIsInvalidated) {
        ...
        val alias = mAESEncryptor.getExtendedKeyStoreAlias(options, options.isAuthenticationRequired)
      ...
      /* Android API 23+ supports storing symmetric keys in the keystore and on older Android
       ...
       back a value.
       */
      val secretKeyEntry: SecretKeyEntry = getOrCreateKeyEntry(SecretKeyEntry::class.java, mAESEncryptor, options, options.isAuthenticationRequired)
